...
import { CHAPTERS, LEVELS, levelById, levelLabel } from './levels'
import { run } from './vm'
...
describe('as 20 fases', () => {
  ...
  it('os capitulos cobrem todas as fases, uma vez cada, em ordem', () => {
    const covered = CHAPTERS.flatMap((c) => c.levelIds)
    expect(covered).toEqual(LEVELS.map((l) => l.id))
  })

  it('rotula fases no formato capitulo-indice', () => {
    expect(levelLabel(1)).toBe('1-1')
    expect(levelLabel(6)).toBe('2-1')
    expect(levelLabel(20)).toBe('4-4')
  })
})
