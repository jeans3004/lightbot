import { VOID, type Chapter, type Cmd, type Dir, type Level, type SlotId } from './types'
...
export const LEVELS: Level[] = build([
  // ---------------------------------------------------------------- basico
  {
    name: 'Primeiro passo',
    intro: [
      'Bem-vindo! (Toque em qualquer lugar para continuar)',
      'Meu trabalho e acender todos os ladrilhos azuis.',
      'Mas eu so faco o que voce programar. Arraste os comandos para o bloco MAIN e toque em Executar!',
    ],
    map: `0 0 0*`,
    start: [0, 0, 0],
    slots: { main: 8 },
  ...
  {
    name: 'Vire',
    intro: ['Girar muda para onde eu olho, mas nao me tira do lugar.'],
    map: `
      0 0 0
      . . 0
      . . 0*
  ...
  // ----------------------------------------------------------------- pulos
  {
    name: 'Salto',
    intro: [
      'Andar so funciona no plano.',
      'Para subir um degrau, eu preciso pular. E pulando eu tambem desco de qualquer altura!',
    ],
    map: `0 1 2 2*`,
    start: [0, 0, 0],
    slots: { main: 8 },
  ...
  // ------------------------------------------------------------ procedimentos
  {
    name: 'Procedimento',
    intro: [
      'O bloco MAIN e pequeno demais para este caminho...',
      'Mas veja o bloco P1! Guarde nele o trecho que se repete, e use o comando P1 para chama-lo quantas vezes quiser.',
    ],
    map: `0* 1 0* 1 0* 1 0*`,
    start: [0, 0, 0],
    slots: { main: 5, p1: 4 },
  ...
  // -------------------------------------------------------------- recursao
  {
    name: 'Recursao',
    intro: [
      'Segredo: um procedimento pode chamar a si mesmo!',
      'Coloque P1 no fim do proprio P1 e ele repete para sempre. Nao se preocupe: a fase acaba assim que a ultima luz acender.',
    ],
...
/** Capitulos, como no menu do original: cada um agrupa um conceito. */
export const CHAPTERS: Chapter[] = [
  { id: 1, name: 'Basico', levelIds: [1, 2, 3, 4, 5] },
  { id: 2, name: 'Pulos', levelIds: [6, 7, 8, 9] },
  { id: 3, name: 'Procedimentos', levelIds: [10, 11, 12, 13, 14, 15, 16] },
  { id: 4, name: 'Recursao', levelIds: [17, 18, 19, 20] },
]
...
/** Rotulo "capitulo-fase", como "1-3", que o original mostra no canto. */
export function levelLabel(levelId: number): string {
  const chapter = chapterOf(levelId)
  return `${chapter.id}-${chapter.levelIds.indexOf(levelId) + 1}`
}
