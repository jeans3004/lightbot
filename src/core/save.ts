...
interface SaveData {
  ...
  audio: AudioMode
  /** Fases cujo tutorial ja foi mostrado. */
  intros: number[]
}

function blank(): SaveData {
  return { version: 1, completed: [], programs: {}, best: {}, audio: 'full', intros: [] }
}
...
function sanitize(raw: unknown): SaveData {
  ...
  if (Array.isArray(data.intros)) {
    out.intros = data.intros.filter((n): n is number => Number.isInteger(n))
  }
  return out
}
...
export const Save = {
  ...
  introSeen(id: number): boolean {
    return read().intros.includes(id)
  },

  markIntroSeen(id: number): void {
    const data = read()
    if (!data.intros.includes(id)) data.intros.push(id)
    write(data)
  },
