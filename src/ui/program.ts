...
export class ProgramEditor {
  ...
  /** Blocos e paleta vivem em cantos diferentes da tela, como no original:
   *  os blocos no painel a direita, a paleta logo abaixo do tabuleiro. */
  constructor(
  ...
  private render(): void {
    ...
    this.paletteEl.innerHTML = level.allowed
      .map((cmd) => {
        ...
        return `<button type="button" class="chip" data-cmd="${cmd}"
  ...
  private stripHtml(slot: SlotId, cells: (Cmd | null)[]): string {
    const slots = cells
      .map((cmd, index) => {
        ...
        const meta = CMD_META[cmd]
        return `<div class="slot filled" data-index="${index}" data-cmd="${cmd}"
    ...
    return `
      <div class="strip" data-slot="${slot}" role="grid" aria-label="Bloco ${SLOT_LABEL[slot]}">
        <div class="strip-head">${SLOT_LABEL[slot]}</div>
