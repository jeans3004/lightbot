...
// Isometrica classica de 30 graus (elevacao = atan(1/sqrt2)), como no original.
const ELEVATION = 0.6155
...
export class View {
  ...
  constructor(private canvas: HTMLCanvasElement) {
    // Fundo transparente: o cenario claro com trilhas de circuito e CSS.
    ...
    // Cena clara e chapada, como um desenho: muita luz ambiente e um sol
    // suave so para separar as faces dos blocos.
    this.scene.add(new THREE.HemisphereLight(0xffffff, 0xc8d4e4, 2.6))
    ...
    // Um pouco maior que a celula sugere: no original o robo domina o ladrilho.
    this.robot.root.scale.setScalar(1.12)
  ...
  private animateLight(step: Step, duration: number): Promise<void> {
    ...
    return this.timeline.run(
      ...
      (k) => {
        ...
        this.robot.antenna.emissiveIntensity = 0.4 + dip * 1.6
      },
      Ease.linear,
    ).then(() => {
      body.scale.set(1, 1, 1)
      this.robot.antenna.emissiveIntensity = 0.4
  ...
  async celebrate(): Promise<void> {
    ...
    await this.timeline.run(
      ...
      (k) => {
        ...
        // olhos se fecham no pico de cada pulo: uma piscada feliz
        const blink = 1 - Math.pow(hop, 6) * 0.9
        for (const eye of this.robot.eyes) eye.scale.y = blink
        this.robot.antenna.emissiveIntensity = 0.4 + hop * 1.4
      },
      Ease.linear,
    )
    ...
    body.scale.set(1, 1, 1)
    for (const eye of this.robot.eyes) eye.scale.y = 1
    this.robot.antenna.emissiveIntensity = 0.4
  }
  ...
  private fitFrustum(): void {
    ...
    const MARGIN = 0.6
    // A paleta ja tem sua propria faixa fora do canvas; aqui so um respiro.
    ...
    // O original mantem o tabuleiro pequeno e cercado de ar, nunca colado
    // nas bordas: fases minusculas nao podem ocupar a tela inteira.
    const MIN_SPAN = 8.5
    ...
    if (top - bottom < MIN_SPAN * 0.7) {
      const cy = (top + bottom) / 2
      bottom = cy - (MIN_SPAN * 0.7) / 2
      top = cy + (MIN_SPAN * 0.7) / 2
    }
